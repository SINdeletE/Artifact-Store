...
DECLARE
        ...
        row_count int;
BEGIN
        SELECT * INTO client_s_i FROM store_items si WHERE si.id = store_item_id;
        IF NOT FOUND THEN
            RAISE EXCEPTION 'STORE ITEM NOT FOUND';
        END IF;

        SELECT * INTO client_b FROM balances b WHERE b.id = balance_id AND b.currency_id = client_s_i.currency_id;
        IF NOT FOUND THEN
        ...
        DELETE FROM store_items WHERE id = store_item_id;
        ...
        -- UPDATE BALANCE
        UPDATE balances SET amount = amount - client_s_i.price WHERE id = balance_id;
        GET DIAGNOSTICS row_count = ROW_COUNT;
        IF row_count = 0 THEN
            RAISE EXCEPTION 'CONFLICT';
        END IF;
        IF (SELECT amount FROM balances WHERE id = balance_id LIMIT 1) < 0 THEN
            RAISE EXCEPTION 'CONFLICT';
        END IF;
